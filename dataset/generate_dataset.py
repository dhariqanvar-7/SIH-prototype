...
def write_data_dictionary(root: Path) -> None:
    rows = [
        ...
        ("ground_truth_observations.geojson", "observation_id", "string", "Observation-area polygon id"),
        ...
        ("gnss_observations.csv", "easting", "float", "EPSG:32643 easting (metres); survey date unknown"),
        ("gnss_observations.csv", "northing", "float", "EPSG:32643 northing (metres); survey date unknown"),
    ...
    for rel, h in sorted(sig.items()):
        ...
        if ext in {".gpkg", ".geojson"}:
            ...
            try:
                ...
                pass
        if p.name == "legacy_parcels_no_crs.geojson":
            entry["crs"] = None
