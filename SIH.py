...
def _geom_bytes(g) -> bytes:
    """Stable, version-tolerant byte representation of a geometry."""
    if g is None or g.is_empty:
        return b"EMPTY"
    # Shapely 2.0: to_wkb accepts rounding_precision
    try:
        return shapely.to_wkb(g, rounding_precision=4)
    except TypeError:
        pass
    # Shapely 2.1+: rounding_precision removed; try WKT with rounding
    try:
        return shapely.to_wkt(g, rounding_precision=4).encode("utf-8")
    except TypeError:
        pass
    # Last resort: plain WKB (deterministic for our fixed-seed inputs)
...
def content_hash(path: Path) -> str:
    ...
    GPKG embeds timestamps and internal layout, so we canonicalise by
    re-reading rows and hashing a normalised representation.
    """
    if path.suffix.lower() == ".gpkg":
        gdf = gpd.read_file(path)
        cols = sorted(c for c in gdf.columns if c != "geometry")
        if cols:
            gdf = gdf.sort_values(by=cols).reset_index(drop=True)
        h = hashlib.sha256()
        for _, row in gdf.iterrows():
            for c in gdf.columns:
                if c == "geometry":
                    h.update(_geom_bytes(row.geometry))
                else:
                    h.update(str(row[c]).encode("utf-8"))
        return h.hexdigest()
